...
const Card = ({card,UserRole})=>{
    ...
    <Link to={`/card/${(card.name).replace(/\s+/g, '')}/${card._id}`} className='py-1 block'>
        <div className='group w-full hover:border-3 border-transparent hover:border-[#49C5C5] bg-gray-50 cursor-pointer rounded-lg shadow-[0px_0px_3px_rgba(12,12,13,0.4)] overflow-hidden'>
